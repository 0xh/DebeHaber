                ...
                <div class="m-portlet m-portlet--full-height m-portlet--fit ">
                    ...
                    <div class="m-portlet__body">
                        ...
                        <div class="m-widget4 m-widget4--chart-bottom" style="min-height: 480px">
                            ...
                            @if(isset($company_subscription))
                                @foreach($company_subscription as $taxPayer)
                                    <div class="m-widget4__item">
                                        <div class="m-widget4__img m-widget4__img--logo">
                                        ...
                                        <div class="m-widget4__info">
                                            <span class="m-widget4__title">
                                                <a href="{{url('selectCompany', $taxPayer->company)}}">
                                                    {{ $taxPayer->company->name }}
                                                </a>
                                            </span><br>
                                            <span class="m-widget4__sub">
                                                {{ $taxPayer->company->alias }} | {{ $taxPayer->company->gov_code }}
                                            </span>
                                        </div>
                                        <div class="m-btn-group m-btn-group--pill btn-group" role="group" aria-label="...">

                                            <a href="{{url('selectCompany', $taxPayer->company)}}" class="m-btn btn btn-secondary">
                                                <i class="la la-info text-info"></i>
                                            </a>

                                            @if ($favorites->contains('company_id', $taxPayer->company_id))
                                                <a href="#" onclick="addFavorite({{$taxPayer->company_id}},0)" class="m-btn btn btn-secondary">
                                                    <i class="la la-star text-warning"></i>
                                                </a>
                                                {{-- <input type="checkbox" class="icheck" data-checkbox="icheckbox_square-blue" checked="" onclick="addFavorite({{$taxPayer->company_id}},0)"> --}}
                                            @else
                                                <a href="#" onclick="addFavorite({{$taxPayer->company_id}},1)" class="m-btn btn btn-secondary">
                                                    <i class="la la-star-o text-warning"></i>
                                                </a>
                                                {{-- <input type="checkbox" class="icheck" data-checkbox="icheckbox_square-blue" onclick="addFavorite({{$taxPayer->company_id}},1)"> --}}
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
